...
*   **版本演进 (Changelog)**:
    *   **v3.3.22 (2026-01-12)**:
        - **配额保护系统升级 (Quota Protection Enhancement)**:
            - **模型级精准监控**: 新增监控模型筛选功能。支持针对 `gemini-3-flash`, `gemini-3-pro-high`, `claude-sonnet-4-5` 三个核心模型进行独立勾选。系统仅在勾选模型额度低于阈值时触发保护，避免非核心模型耗尽导致的账号误锁。
            - **判定逻辑优化**: 保护触发逻辑从“平均配额”升级为“勾选模型的最小配额”。只要选中的任意一个关键模型跌破阈值（如 20%），系统将立即为了安全禁用该账号。
            - **UI 强制约束**: 在设置中开启保护时自动默认勾选 `claude-sonnet-4-5`；UI 层面强制要求至少保持 1 个模型勾选，确保保护逻辑始终生效。
        - **全自动“刷新-保护-预热”联动 (Automated Workflow)**:
            - **强制自动刷新**: 隐藏并强制开启“后台自动刷新”开关。作为自动化闭环的心脏，确保系统始终拥有最新配额数据。
            - **瞬间联动**: 刷新配额后，系统会自动根据状态执行：账号保护 -> 账号恢复 -> 瞬间智能预热，实现全生命周期自动化管理。
        - **智能预热支持自定义勾选 (Customizable Smart Warmup)**:
            - **模型级灵活配置**: 仿照配额保护，智能预热现在也支持自定义勾选模型。支持 `gemini-3-flash`, `gemini-3-pro-high`, `claude-sonnet-4-5`, `gemini-3-pro-image` 的独立预热配置。
            - **UI 体验升级**: 抽离了独立的 `SmartWarmup.tsx` 组件，提供与配额保护一致的勾选体验。
            - **默认全选逻辑**: 为保障新用户的并发就绪能力，开启预热时默认勾选所有核心模型，并实施“至少勾选一个”的 UI 约束。
            - **调度效率优化**: 优化了后端调度器对配置的实时读取机制，确保修改模型勾选后 100% 立即生效。
        ...
        - **逻辑稳定性修复**: 修复了旧逻辑中可能出现的无效变量引用及所有权冲突，显著提升了高并发调度下的稳定性。
    *   **v3.3.21 (2026-01-11)**:
